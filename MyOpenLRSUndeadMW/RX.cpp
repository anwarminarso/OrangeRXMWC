...
#define AVERAGING_ARRAY_LENGTH 4
	...
	static uint16_t Servo_Buffer[8] = {3000,3000,3000,3000,3000,3000,3000,3000};   //servo position values from RF
	...
	void Hopping(void) {
		hopping_channel++;
		if (hopping_channel>CHNO) hopping_channel = 0;
	...
	void Read_OpenLRS_RC() {
		...
		static uint8_t rssiCounter = 0;
		static uint8_t rssiValue = 0;
		...
		if(nIRQ_0) { // RFM22B INT pin Enabled by received Data
			...
			for (int i = 0; i < 8; i++)
				Servo_Buffer[i] = map(read_8bit_data(),0,200,650,2000);

			rssiValue += _spi_read(0x26);  // Read the RSSI value
			rssiCounter++;
			if (rssiCounter >= 20) {
			...
			rcData[THROTTLE] = Servo_Buffer[1];
			rcData[PITCH] = Servo_Buffer[2];
